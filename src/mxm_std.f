...
c
     ...
     $   call mxmtest(s(1,1, 9,k),nn,c(k, 9),madd  ,'madd ',k,ivb)
      ...
      call comm_barrier(comms_h)
      if (comms_id.eq.0) call mxm_analyze(s,a,nn,c,nt,ivb)
      call comm_barrier(comms_h)
...
c-----------------------------------------------------------------------
      ...
      save    ll,icalld
      data    ll,icalld  /1,0/
      real*8 time1, time2, overh, t0 

      if (icalld.eq.0) then    !     Initialize matrices:
         icalld = icalld + 1
         call comm_time(time1)
         call initab(a,b,lt)
         call comm_time(time2)
         time2 = time2-time1
         if (comms_id.eq.0) write(6,*) 'mxm test init:',lt,time2,name
      endif
...
c
          timel = time/loop
          if (comms_id.eq.0) write(6,199) n,n1,n2,n3,flops,timel,name
...
c
          ...
          timel = time/loop
c
          if (comms_id.eq.0) write(6,198) n,n1,n2,n3,flops,timel,name
