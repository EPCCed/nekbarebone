c-----------------------------------------------------------------------
      subroutine cg(x,f,g,c,r,w,p,z,n,niter)
      include 'TOTAL'
...
c     set machine tolerances
      ...
      iter = 0
      if (comms_id.eq.0)  write(6,6) iter,rnorm
...
c     call tester(z,r,n)  
      do iter=1,miter
         ...
         rnorm = sqrt(rtr)
c        if (comms_id.eq.0.and.mod(iter,100).eq.0) 
...
c        if (rtr.le.rlim2) goto 1001
 ...
 1001 continue
      ...
      if (comms_id.eq.0) write(6,6) iter,rnorm,alpha,beta,pap
...
c-----------------------------------------------------------------------
      ...
      if (comms_id.eq.0) w(1) = 0.  ! suitable for solvability
