...
C     find the Median of the (global) set A
      ...
      N2 = (NTOT+1)/2
      IF (.NOT.IFOK) THEN
        WRITE(6,8) COMMS_ID,N,(A(I),I=1,N)
        WRITE(6,9) COMMS_ID,NTOT,N2,N,GMIN,GMAX
...
C     Invoke vector reduction across processors:
      ...
      NGT=WORK1(4)
      IF (.NOT.IFOK) THEN
         WRITE(6,101) COMMS_ID,GUESS,CLT,CGT
         WRITE(6,102) COMMS_ID,(WORK1(I),I=1,5)
...
C           the median is (usually) one of the values 
      ...
      ENDIF
       IF (.NOT.IFOK) WRITE(6,*) COMMS_ID,'FMDIAN2',FMDIAN,(A(I),I=1,N)
      return
C
C     Error handling
C
 9000 CONTINUE
   ...
   11 FORMAT('ABORTING IN FMDIAN: N,AMIN,AMAX:',I6,3G14.6)
      DO 13 I1=1,N,5
        IN=I1+5 
        IN=MIN(IN,N)
        WRITE(6,12) COMMS_ID,(A(I),I=I1,IN)
   12   FORMAT(I4,' FMA:',5G14.6)
   13 CONTINUE
      DO 15 I1=1,ITRY,5
        IN=I1+5
        IN=MIN(IN,ITRY)
        WRITE(6,14) COMMS_ID,(GUES(I),I=I1,IN)
